...
[Interface]
Address = 172.16.11.1/24$([[ -n "$ip6" ]] && echo ", fddd:2c4:2c4:2c4::1/64")
...
create_firewall_rules() {
	if systemctl is-active --quiet firewalld.service; then
		...
		firewall-cmd -q --zone=trusted --add-source=172.16.11.0/24
		firewall-cmd -q --permanent --add-port="$port"/udp
		firewall-cmd -q --permanent --zone=trusted --add-source=172.16.11.0/24
		# Set NAT for the VPN subnet
		firewall-cmd -q --direct --add-rule ipv4 nat POSTROUTING 0 -s 172.16.11.0/24 ! -d 172.16.11.0/24 -j MASQUERADE
		firewall-cmd -q --permanent --direct --add-rule ipv4 nat POSTROUTING 0 -s 172.16.11.0/24 ! -d 172.16.11.0/24 -j MASQUERADE
		if [[ -n "$ip6" ]]; then
...
[Service]
Type=oneshot
ExecStart=$iptables_path -t nat -A POSTROUTING -s 172.16.11.0/24 ! -d 172.16.11.0/24 -j MASQUERADE
ExecStart=$iptables_path -I INPUT -p udp --dport $port -j ACCEPT
ExecStart=$iptables_path -I FORWARD -s 172.16.11.0/24 -j ACCEPT
ExecStart=$iptables_path -I FORWARD -m state --state RELATED,ESTABLISHED -j ACCEPT
ExecStop=$iptables_path -t nat -D POSTROUTING -s 172.16.11.0/24 ! -d 172.16.11.0/24 -j MASQUERADE
ExecStop=$iptables_path -D INPUT -p udp --dport $port -j ACCEPT
ExecStop=$iptables_path -D FORWARD -s 172.16.11.0/24 -j ACCEPT
...
remove_firewall_rules() {
	...
	if systemctl is-active --quiet firewalld.service; then
		ip=$(firewall-cmd --direct --get-rules ipv4 nat POSTROUTING | grep '\-s 172.16.11.0/24 '"'"'!'"'"' -d 172.16.11.0/24' | grep -oE '[^ ]+$')
		# Using both permanent and not permanent rules to avoid a firewalld reload.
		firewall-cmd -q --remove-port="$port"/udp
		firewall-cmd -q --zone=trusted --remove-source=172.16.11.0/24
		firewall-cmd -q --permanent --remove-port="$port"/udp
		firewall-cmd -q --permanent --zone=trusted --remove-source=172.16.11.0/24
		firewall-cmd -q --direct --remove-rule ipv4 nat POSTROUTING 0 -s 172.16.11.0/24 ! -d 172.16.11.0/24 -j MASQUERADE
		firewall-cmd -q --permanent --direct --remove-rule ipv4 nat POSTROUTING 0 -s 172.16.11.0/24 ! -d 172.16.11.0/24 -j MASQUERADE
...
new_client() {
	...
	if [ "$1" = "add_client" ] && [ "$add_client" = 0 ]; then
		...
		if [[ ! "$specify_ip" =~ ^[yY]$ ]]; then
			echo "Using auto assigned IP address 172.16.11.$octet."
		fi
	fi
	if [[ "$specify_ip" =~ ^[yY]$ ]]; then
		echo
		read -rp "Enter IP address for the new client (e.g. 172.16.11.X): " client_ip
		...
		until [[ $client_ip =~ ^10\.7\.0\.([2-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-4])$ ]] \
			...
			if [[ ! $client_ip =~ ^10\.7\.0\.([2-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-4])$ ]]; then
				echo "Invalid IP address. Must be within the range 172.16.11.2 to 172.16.11.254."
			else
...
PresharedKey = $psk
AllowedIPs = 172.16.11.$octet/32$(grep -q 'fddd:2c4:2c4:2c4::1' "$WG_CONF" && echo ", fddd:2c4:2c4:2c4::$octet/128")
# END_PEER $client
EOF
...
[Interface]
Address = 172.16.11.$octet/24$(grep -q 'fddd:2c4:2c4:2c4::1' "$WG_CONF" && echo ", fddd:2c4:2c4:2c4::$octet/64")
DNS = $dns
PrivateKey = $key
